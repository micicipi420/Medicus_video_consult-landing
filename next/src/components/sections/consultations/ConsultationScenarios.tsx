...
export function ConsultationScenarios() {
  return (
    <section className="container mx-auto px-4 lg:px-6 mb-16" id="scenarios">
      ...
      <div className="bg-[var(--glass-section-fill)] backdrop-blur-[var(--glass-section-blur)] rounded-[2.5rem] p-8 md:p-12 border border-white/60 shadow-glass max-w-3xl mx-auto">
        <ul className="space-y-5" role="list">
          {scenarios.map((text) => (
            <li key={text} className="flex items-start gap-4">
              <span className="flex-shrink-0 w-7 h-7 bg-[var(--glass-button-fill)] backdrop-blur-[var(--glass-button-blur)] border border-white/60 rounded-full flex items-center justify-center shadow-glass-inner-strong mt-0.5">
