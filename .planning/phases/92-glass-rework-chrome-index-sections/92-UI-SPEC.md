...
| Tokens consumed | `--glass-{section,card,form,button}-{fill,blur}` (Phase 90), `--blob-x`, `--blob-y`, `--blob-heat`, `--blob-velocity` (Phase 91 runtime), `--mu-cta-from`/`--mu-cta-to` (CTA gradient — opaque-forever; runtime CSS holds v6 values `#0E8FB5 → #3B6DD0`). The v1.3 gradient (`#1AC67E → #0D9DB5`) lives only as `cta-gradient-from`/`-to` keys в DESIGN.md YAML — нет runtime CSS-токенов; если v1.3 потребуется в Phase 92 sweep — вызывайте hex напрямую с пометкой в комментарии (анти-паттерн флаг — обсудить в 92-08). |
...
> **Inherited / FROZEN — checker exception.** Эта таблица документирует scale, унаследованную из shipped Phase 79. Phase 92 не вводит ни новых size-ролей, ни новых weight-значений — каждая строка имеет «Phase 92 mutation: none» (за единственным исключением promotion form label из weight 400 → 700, sanctioned in Decision B / GLASS-07). Превышение 4-size / 2-weight рекомендации UI-checker — артефакт inheritance, не нового design choice. Verification reference only.
...
| Accent 10% (CTA / brand) | runtime CSS gradient `#0E8FB5 → #3B6DD0` (`--mu-cta-from`/`--mu-cta-to` — v6 палитра; используется во всех `/` index CTA). v1.3 палитра `#1AC67E → #0D9DB5` существует только как DESIGN.md YAML-ключи `cta-gradient-from`/`-to` (нет runtime токена; не используется на index в Phase 92 scope — проверка через grep `1AC67E\|0D9DB5` в 92-08). | **Reserved for the 7-component CTA master list (Phase 90).** No glass, no `backdrop-filter`, no opacity transparency. |
...
7. Any future component using `.btn-primary` / `.liquid-btn-primary` / v6 gradient utility classes (`from-mu-cta-from`, `to-mu-cta-to` — Tailwind utilities, generated by `@theme` block в globals.css из v6-окрашенных CSS-токенов)
...
**Tier validation rule (anti-pattern #4):** any computed `background-color` final fill > 0.16 в Phase 92 sweep — defect для НЕформенных surfaces. Двa санкционированных исключения:

1. **Tier 3 button** = 0.12 desktop / 0.16 mobile — единственная non-form occurrence на ровно 0.16.
2. **Tier 2 form panel** = 0.14 desktop / 0.18 mobile — это GLASS-07 form-safety floor (TZ §13). Допустимо до 0.30 при escalation с `KD-v9-002` (Decision G). Anti-pattern #4 НЕ применяется к `ContactForm`/`ContactSection` form-panel; применяется ко всем остальным surfaces включая Tier 1 hover (Tier 2 hover на cards = 0.14 desktop / 0.18 mobile — попадает в FORM-SAFETY-семейство exception только если используется form fill token; на cards-hover используется card-tier ramp, обычно ≤0.16).
