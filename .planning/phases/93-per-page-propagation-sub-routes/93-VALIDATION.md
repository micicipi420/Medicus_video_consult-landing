...
| 93-03-01 | 03 | 2 | ROUTE-02 | — | `/consultations` (7 rendered files) swept; same gates as checkup | full | `pnpm --dir next build && pnpm --dir next playwright test --grep "consultations"` | ✅ | ⬜ pending |
| 93-04-01 | 04 | 2 | ROUTE-03 | — | `/treatment-abroad` (4 files) swept; same gates | full | `pnpm --dir next build && pnpm --dir next playwright test --grep "treatment-abroad"` | ✅ | ⬜ pending |
| 93-05-01 | 05 | 2 | ROUTE-04 | — | `/contacts` (page.tsx + actually-imported files) verify-only audit; dead-code (`contacts/*` 4 files) untouched per Decision F; produces 93-05-CONTACTS-AUDIT.md | docs | `pnpm --dir next build && grep -q 'Decision F' .planning/phases/93-per-page-propagation-sub-routes/93-05-CONTACTS-AUDIT.md` | ✅ | ⬜ pending |
| 93-06-01 | 06 | 3 | ROUTE-06 | — | shadcn primitives (`card`, `dialog`, `input`, `select`, `textarea`) — verify admin-only consumer (`admin/submissions-table.tsx`); future-proof note on `dialog.tsx`; produces 93-06-SHADCN-AUDIT.md | docs | `grep -q 'admin-only' .planning/phases/93-per-page-propagation-sub-routes/93-06-SHADCN-AUDIT.md` | ✅ | ⬜ pending |
| 93-07-01 | 07 | 3 | ROUTE-04, ROUTE-05 | — | Per-route Playwright visual-diff vs Wave 0 baseline across all 4 sub-routes × 2 breakpoints; no regression in non-blob regions | full | `pnpm --dir next playwright test --project=visual-regression` | ✅ | ⬜ pending |
| 93-07-02 | 07 | 3 | ROUTE-07 | T-93-07-01 (form submission integrity) | Submission-path E2E: submit one test record per route; confirm Directus arrival; cleanup test record. **<read_first>** must enumerate all required ContactForm fields (name, phone, specialization dropdown, optional description) AND LeadFormSection field shape before authoring spec body — guards against false-positive failures | full | `pnpm --dir next playwright test --grep "submission-e2e"` | ❌ W0 (creates spec) | ⬜ pending |
| 93-07-03 | 07 | 3 | ROUTE-04, ROUTE-05, ROUTE-06, ROUTE-07 | — | DESIGN.md anti-pattern register: append #16 "Do not propagate ContactForm honeypot/timing pattern without explicit anti-bot decision phase" | docs | `grep -q 'ContactForm honeypot/timing' DESIGN.md` | ❌ W0 (Wave 3 task) | ⬜ pending |
| 93-07-04 | 07 | 3 | ROUTE-04, ROUTE-05, ROUTE-06, ROUTE-07 | — | 93-SWEEP-AUDIT.md close-out audit (mirrors 92-08-SWEEP-AUDIT.md format) — token-consumption counts, CTA invariant negative-grep, visual-diff status, submission-E2E status | docs | `test -f .planning/phases/93-per-page-propagation-sub-routes/93-SWEEP-AUDIT.md` | ❌ W0 (Wave 3 task) | ⬜ pending |
