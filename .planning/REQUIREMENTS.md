...
- [x] **FND-03**: A11y `@layer` block in `liquid-glass.css` enumerates every glass class name (current + new) under `prefers-reduced-motion`, `prefers-reduced-transparency`, and `prefers-contrast: more` — single source of truth
...
- [x] **FND-06**: `<div class="living-blob-field" aria-hidden="true">` skeleton mounted in `next/src/app/layout.tsx` with seeded `:root` CSS vars; `MeshBackground.tsx` removed
